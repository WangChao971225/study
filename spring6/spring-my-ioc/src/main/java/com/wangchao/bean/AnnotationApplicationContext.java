...
public class AnnotationApplicationContext implements ApplicationContext{
    ...
    private void loadBean(File file) throws Exception {
        ...
        if (file.isDirectory()) {
            //2 获取文件夹类的所有内容
            File[] chlidrenFiles = file.listFiles();

            //3 判断文件夹内是否为空，空直接返回
            if (chlidrenFiles == null || chlidrenFiles.length == 0) {
                return;
            }

            //4 如果文件夹里面不为空，遍历文件夹所有内容
            for (File child: chlidrenFiles) {
                //4.1 遍历每个File文件，继续判断，如果还是文件夹，递归
                    ...
                    if (pathWithClass.contains(".class")) {
                        ...
                        if (!clazz.isInterface()) {
                            ...
                            if (annotation != null) {
                                ...
                                //4.7 把对象实例化之后，当到map集合beanFactory
